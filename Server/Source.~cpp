      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
       ...
       if(Creion->Down)
        ...
        if(Galetusa->Down)
        {
            ...
            Interfata->srvSocket->Socket->Connections[0]->SendText(SendGaletusa);
            SendGaletusa="";
        }
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
        ...
        if(Creion->Down)
        {
                ...
                SendLinie+=" ";
                Interfata->srvSocket->Socket->Connections[0]->SendText(SendLinie);
                SendLinie="";
        }
        ...
        if(Guma->Down)
        {
                ...
                Panza->Canvas->Pen->Width=0;
                SendGuma+=" ";
                Interfata->srvSocket->Socket->Connections[0]->SendText(SendGuma);
                SendGuma="";
        }

        Joc->fig->Desen();

        Interfata->srvSocket->Socket->Connections[0]->SendText(SendDreptunghi);
        Interfata->srvSocket->Socket->Connections[0]->SendText(SendCerc);
        SendCerc="";
        SendDreptunghi="";
}
//---------------------------------------------------------------------------
...
void __fastcall TInterfata::ClearClick(TObject *Sender)
{
        ...
        Interfata->srvSocket->Socket->Connections[0]->SendText(SendClear);
}
//---------------------------------------------------------------------------
...
void __fastcall TInterfata::RaspunsKeyPress(TObject *Sender, char &Key)
{
        if(Key==13)
        {
                 ...
                 else if(Raspuns->Text == Cuvant->Caption)
                 {
                      ...
                      (Joc->srv_scor)++;
                      srvSocket->Socket->Connections[0]->SendText("@");
                      Raspuns->Enabled=false;
                      srvSocket->Socket->Connections[0]->SendText("R AM GHICIT!");
                 ...
                 else
                 {
                      ...
                      srvSocket->Socket->Connections[0]->SendText(SendRaspuns);
...
void __fastcall TInterfata::Timer1Timer(TObject *Sender)
{
        Joc->RandomCuvant();
        Joc->Play();
...
void __fastcall TInterfata::srvSocketClientRead(TObject *Sender,
